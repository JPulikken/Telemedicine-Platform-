  ...
  try {
    ...
    if (response.ok) {
      ...
      const fullAppointmentDetails = {
        ...
        hospital: {
          name: selectedDoctorDetails.hospital, // Assuming hospital is a string
          address: "Dr.Chandamma Sagar Institution,DEVARAKAGGALAHALLI, HAROHALLI, Kanakapura Main Rd, Kanakapura, Karnataka 562112" // Set the fixed address here
        },
        patient: {
            name: patientUserName // Use the name stored during login/registration
        },
        status: data.appointment.status, // Get status from backend response
